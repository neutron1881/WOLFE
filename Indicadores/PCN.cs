...
namespace ATAS.Indicators.Technical
{
    ...
    public class SpyCsvDeltaNet : Indicator
    {
        ...
        // Track previous raw value per key for alarm comparison
        private readonly Dictionary<string, decimal?> _prevRawByKey = new(StringComparer.OrdinalIgnoreCase);
        ...
        [Category("Datos"), Display(Name = "Columna principal (serie1)", Order =4)]
        ...
        public string PrimaryColumn
        {
            ...
            set
            {
                ...
                if (_defs.Count >0)
                    _defs[0].Key = _deltaColumn; // actualizar clave de serie1
                ForceReload();
            }
        }
        ...
        [Category("Visual"), Display(Name = "Color serie1", Order =11)]
        ...
        [Category("Visual"), Display(Name = "Mostrar línea0", Order =13)]
        ...
        [Category("Visual"), Display(Name = "Color línea0", Order =14)]
        public Color ZeroLineColor { get => _zeroLineColor; set { _zeroLineColor = value; RedrawChart(); } }

        // Enum para fuentes de alarma (desplegable)
        ...
        private string ResolveAlarmKey(AlarmSource src)
        => src switch
        {
            AlarmSource.Primary => PrimaryColumn,
            AlarmSource.NetDelta => "Net Delta",
            AlarmSource.CallVol => "Call Vol",
            AlarmSource.PutVol => "Put Vol",
            AlarmSource.NetGex => "Net Gex",
            AlarmSource.TotalIO => "Total IO",
            AlarmSource.NetIO => "Net IO",
            AlarmSource.CallOI => "Call OI",
            AlarmSource.PutOI => "Put OI",
            AlarmSource.NetVol => "Net Vol",
            AlarmSource.CashCall => "Cash Call",
            AlarmSource.CashPut => "Cash Put",
            AlarmSource.IVCall => "IV Call",
            AlarmSource.IVPut => "IV Put",
            AlarmSource.CallDelta => "Call Delta",
            AlarmSource.DeltaPut => "Delta Put",
            AlarmSource.CashNeto => "Cash Neto",
            AlarmSource.IVNeta => "IV Neta",
            AlarmSource.DeltaNeto => "Delta Neto",
            AlarmSource.CallAsk => "CallAsk",
            AlarmSource.CallBid => "CallBid",
            AlarmSource.PutAsk => "PutAsk",
            AlarmSource.PutBid => "PutBid",
            AlarmSource.TotalCall => "TotalCall",
            AlarmSource.TotalPut => "TotalPut",
            AlarmSource.SNeto => "SNeto",
            _ => PrimaryColumn
        };

        // Alarmas configurables (% de cambio entre lecturas consecutivas)
        private bool _alarm1Enabled = false;
        private AlarmSource _alarm1Source = AlarmSource.Primary;
        private decimal _alarm1Percent =5m;

        private bool _alarm2Enabled = false;
        private AlarmSource _alarm2Source = AlarmSource.Primary;
        private decimal _alarm2Percent =10m;

        private bool _alarmPlaySound = true;
        private string _lastAlarmMessage = string.Empty;

        [Category("Alarmas"), Display(Name = "Alarm1 - Activada", Order =0)]
        public bool Alarm1Enabled { get => _alarm1Enabled; set { _alarm1Enabled = value; } }
        [Category("Alarmas"), Display(Name = "Alarm1 - Serie", Order =1)]
        public AlarmSource Alarm1Source { get => _alarm1Source; set { _alarm1Source = value; } }
        [Category("Alarmas"), Display(Name = "Alarm1 - % cambio", Order =2)]
        [Range(0.01,100000)]
        public decimal Alarm1Percent { get => _alarm1Percent; set { _alarm1Percent = Math.Max(0.01m, value); } }

        [Category("Alarmas"), Display(Name = "Alarm2 - Activada", Order =3)]
        public bool Alarm2Enabled { get => _alarm2Enabled; set { _alarm2Enabled = value; } }
        [Category("Alarmas"), Display(Name = "Alarm2 - Serie", Order =4)]
        public AlarmSource Alarm2Source { get => _alarm2Source; set { _alarm2Source = value; } }
        [Category("Alarmas"), Display(Name = "Alarm2 - % cambio", Order =5)]
        [Range(0.01,100000)]
        public decimal Alarm2Percent { get => _alarm2Percent; set { _alarm2Percent = Math.Max(0.01m, value); } }

        [Category("Alarmas"), Display(Name = "Reproducir sonido", Order =6)]
        public bool AlarmPlaySound { get => _alarmPlaySound; set { _alarmPlaySound = value; } }

        [Category("Alarmas"), Display(Name = "Última alarma", Order =7)]
        public string LastAlarmMessage => _lastAlarmMessage;
 ...
 d.Series[bar] = ema;
 // Actualizar valor previo para alarmas
 _prevRawByKey.TryGetValue(d.Key, out var prev);
 if (!_prevRawByKey.ContainsKey(d.Key))
 _prevRawByKey[d.Key] = raw; // inicializa
 }

 // Comprobar alarmas después de haber leído los valores actuales
 CheckAlarm(_alarm1Enabled, ResolveAlarmKey(_alarm1Source), _alarm1Percent, ts);
 CheckAlarm(_alarm2Enabled, ResolveAlarmKey(_alarm2Source), _alarm2Percent, ts);
 }

 private void CheckAlarm(bool enabled, string sourceKey, decimal thresholdPercent, DateTime ts)
 {
 if (!enabled) return;
 if (string.IsNullOrWhiteSpace(sourceKey)) return;

 var key = sourceKey;
 var current = GetValueAt(key, ts);
 _prevRawByKey.TryGetValue(key, out var prev);

 if (prev.HasValue)
 {
 // Evitar división por0: si prev ==0, omitir esta comparación
 if (prev.Value !=0m)
 {
 var pct = (current - prev.Value) / Math.Abs(prev.Value) *100m;
 if (Math.Abs(pct) >= thresholdPercent)
 {
 var dir = pct >=0 ? "sube" : "baja";
 var msg = $"ALERTA {key}: {dir} {Math.Abs(pct):0.##}% (prev={prev.Value:0.###}, cur={current:0.###})";
 _lastAlarmMessage = msg;
 _status = msg;
 if (_alarmPlaySound)
 {
 try { SystemSounds.Exclamation.Play(); } catch { }
 }
 // Reset de referencia para evitar múltiples alarmas por el mismo salto
 _prevRawByKey[key] = current;
 RedrawChart();
 return;
 }
 }
 }
 // Actualizar referencia si no había o no disparó
 _prevRawByKey[key] = current;
 }
 ...
 // Dibujar línea horizontal en0 en el panel del indicador
 try
 {
 int x1 =0;
