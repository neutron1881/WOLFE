...
[DisplayName("Daily Lines Mod")]

public class DailyLinesMod : Indicator
{
    #region Nested types
    ...
    [Display(ResourceType = typeof(Strings), Name = nameof(Strings.Line), GroupName = "Mid",
        Description = nameof(Strings.PenSettingsDescription), Order = 350)]
    public PenSettings MidPen { get; set; } = new() { Color = DefaultColors.Red.Convert(), Width = 2 };

    [Display(ResourceType = typeof(Strings), Name = nameof(Strings.Text), GroupName = "Mid", Description = nameof(Strings.LabelTextDescription),
        Order = 355)]
    ...
    protected override void OnRender(RenderContext context, DrawingLayouts layout)
    {
        ...
        var mid = (range.HighPrice + range.LowPrice) / 2;
        if (mid >= low && mid <= high)
            DrawLevel(context, MidPen, range.OpenBar, mid, MidText, "Mid", periodStr);
    }
